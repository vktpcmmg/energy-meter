...
def plot_waveforms(Vrms, Irms, angle_rad, time_vector):
    ...
    # RMS values directly for waveforms
    v_t = Vrms * np.sqrt(2) * np.sin(omega * time_vector)  # Convert RMS to Peak for waveform
    i_t = Irms * np.sqrt(2) * np.sin(omega * time_vector + angle_rad)  # Convert RMS to Peak
    p_t = v_t * i_t
...
if st.button("Calculate"):
    ...
    v_t = Vrms * np.sqrt(2) * np.sin(omega * t)
    i_t = Irms * np.sqrt(2) * np.sin(omega * t + angle_rad)
    p_t = v_t * i_t

    # Average values
    P = np.mean(p_t)
    S = Vrms * Irms
    # Ensure reactive power is 0 when angle is 0
    if angle == 0:
        Q = 0
    else:
        Q = np.sqrt(S**2 - P**2)
