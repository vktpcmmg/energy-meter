...
def plot_vectors(Vrms_list, Irms_list, current_angles_deg):
    ...
    # Plot voltage and current vectors
    for i in range(3):
        Vx = Vrms_list[i] * np.cos(np.radians(voltage_angles_deg[i]))
        Vy = Vrms_list[i] * np.sin(np.radians(voltage_angles_deg[i]))
        Ix = Irms_list[i] * np.cos(np.radians(voltage_angles_deg[i] + current_angles_deg[i]))
        Iy = Irms_list[i] * np.sin(np.radians(voltage_angles_deg[i] + current_angles_deg[i]))
        
        ax.quiver(0, 0, Vx, Vy, angles='xy', scale_units='xy', scale=1, color=colors[i], label=f'Voltage {labels[i]}', linestyle='solid', width=0.005)
        ax.quiver(0, 0, Ix, Iy, angles='xy', scale_units='xy', scale=1, color=colors[i], label=f'Current {labels[i]}', linestyle='dashed', width=0.005)
...
Irms_R = st.number_input("Phase R Current (RMS) [A]", value=10.0)
Irms_Y = st.number_input("Phase Y Current (RMS) [A]", value=10.0)
Irms_B = st.number_input("Phase B Current (RMS) [A]", value=10.0)
