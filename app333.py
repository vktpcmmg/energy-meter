...
def plot_waveforms(Vrms, Irms, angles_rad, time_vector):
    ...
    v_t = [Vpeak[i] * np.sin(omega * time_vector) for i in range(3)]  # Voltage waveform (using peak value)
    i_t = [Ipeak[i] * np.sin(omega * time_vector - angles_rad[i]) for i in range(3)]  # Current waveform (using peak value)
...
if st.button("Calculate"):
    ...
    plot_waveforms([Vrms_R, Vrms_Y, Vrms_B], [Irms_R, Irms_Y, Irms_B], np.radians([angle_R, angle_Y, angle_B]), time_vector)
