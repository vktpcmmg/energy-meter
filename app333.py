...
def plot_vectors(v_rms, i_rms, angles_deg):
    ...
    # Fixed phase voltages (120 degrees apart)
    for i, angle_deg in enumerate([0, 120, 240]):
        # Voltage vectors are fixed at 0°, 120°, and 240° phase shifts
        angle_rad = np.radians(angle_deg)
        Vx = v_rms[i] * np.cos(angle_rad)
        Vy = v_rms[i] * np.sin(angle_rad)
        ax.quiver(0, 0, Vx, Vy, angles='xy', scale_units='xy', scale=1, color=colors[i], label=f'Voltage {labels[i]}', width=0.005)
    
    # Adjust current vectors based on the slider input
    for i, angle_deg in enumerate(angles_deg):
        # Current vectors are rotated based on the provided phase angle for current
        angle_rad = np.radians(angle_deg)
...
angle_Y = st.slider("Phase Y Current Angle (°)", -180, 180, -120)
angle_B = st.slider("Phase B Current Angle (°)", -180, 180, 120)
...
if st.button("Calculate"):
    ...
    # Plot the vectors and waveforms
    plot_vectors([Vrms_R, Vrms_Y, Vrms_B], [Irms_R, Irms_Y, Irms_B], [angle_R, angle_Y, angle_B])
    
    time_interval_sec = time_interval * 3600  # Convert hours to seconds
    time_vector = np.linspace(0, time_interval_sec, 1000)  # Time vector for waveform plotting
